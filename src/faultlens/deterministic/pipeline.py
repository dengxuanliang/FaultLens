...
def analyze_cases_deterministically(cases: List[Dict[str, Any]], execution_timeout: int = 10) -> List[Dict[str, Any]]:
    ...
    for case in cases:
        ...
        diff_summary = summarize_canonical_diff(
            record.get("reference", {}).get("canonical_code_text"),
            extracted.primary_code_text,
        )
        ...
        if record.get("case_status") == "attributable_failure" and extracted.primary_code_text and language.primary:
            ...
            try:
                runner_result = runner.run(
                    extracted.primary_code_text,
                    record.get("reference", {}).get("test_code_text") or "",
                    execution_timeout,
                )
            except ValueError:
                runner_result = None
